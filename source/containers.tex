...
at \tcode{p} using \tcode{args}. The default \tcode{construct} in \tcode{std::allocator} calls
\begin{codeblock}
...
\pnum
The remainder of this section describes requirements on container classes.
In the context of these requirements, descriptive identifiers are used with
the semantics described by Table~\ref{tab:containers.container.iddef}.
Each requirement is described as a primary lexical fragment followed by additional
constraints. A container class meets the general container requirements if each of
the lexical fragments is well-formed and satisfies the constraints.

\begin{itemize}
\item
\tcode{X::const_iterator} shall denote a type, and the type shall be a constant iterator type
that meets the forward iterator requirements, and whose value type is \tcode{T}.

\item
\tcode{X::difference_type} shall denote a type, and the type shall be a signed integer type
that is identical to the difference type of \tcode{X::iterator} and \tcode{X::const_iterator}.

\item
\tcode{X::size_type} shall denote a type, and the type shall be an unsigned integer type that
can represent any non-negative value of \tcode{difference_type}.

\item
\tcode{X u;} shall be a declaration statement.

\item
\tcode{X u(a);} and \tcode{X u = a;} shall be declaration statements.

\requires \tcode{T} is \tcode{CopyInsertable} into \tcode{X}.

\postconditions \tcode{u == a} shall be true.

\complexity Linear in \tcode{a.size()}.

\item
\tcode{X u(rv)} and \tcode{X u = rv} shall be declaration statements.

\postconditions \tcode{u} shall be equal to the value that \tcode{rv} had before this construction.

\complexity Linear in \tcode{a.size()} when \tcode{X} is is a specialization of \tcode{array}, otherwise constant.

\item
\tcode{v = a} shall denote an expression, and the expression shall be an lvalue of type \tcode{X}.

\postconditions \tcode{v == a} shall be true.

\complexity Linear in \tcode{v.size()}.

\item
\tcode{v = rv} shall denote an expression, and the expression shall be an lvalue of type \tcode{X}.

\remarks All existing elements of \tcode{v} are either move-assigned to or destroyed.
\end{itemize}
